...
(function () {
  ...
  CountDown.prototype.createCountDown = function () {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      timeValue.setAttribute("class",
          "countdown__value countdown__value--" + i + " js-countdown__value--"
          + i);
  ...
  CountDown.prototype.getEndTime = function () {
    ...
    if (this.element.getAttribute("data-timer")) {
      return Number(
              this.element.getAttribute("data-timer")) * 1000
          + new Date().getTime();
  ...
  CountDown.prototype.updateCountDown = function (bool) {
    ...
    if (days === 1) {
      labels[0] = this.labels[0].replace(/s$/, "");
    }
    if (hours === 1) {
      labels[1] = this.labels[1].replace(/s$/, "");
    }
    if (mins === 1) {
      labels[2] = this.labels[2].replace(/s$/, "");
    }
    if (seconds === 1) {
      labels[3] = this.labels[3].replace(/s$/, "");
    }
    ...
    if (this.element.classList.contains("cd-no")) {
      const countdownItems = this.element.getElementsByClassName(
          "countdown__item");
      ...
      for (let i = 0; i < countdownItems.length; i++) {
        ...
        if (next && (next.classList.contains("countdown__custom")
            || next.classList.contains("countdown__custom--element"))) {
