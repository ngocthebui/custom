$(window).on("load", function () {
  $(".tf-swiper").each(function (index, element) {
    ...
    var mobileSm = $this.data("mobile-sm") !== undefined ? $this.data(
        "mobile-sm") : mobile;
    ...
    if (spacing !== undefined && spacingMd === undefined && spacingLg
        === undefined) {
      spacingMd = spacing;
      spacingLg = spacing;
    } else if (spacing === undefined && spacingMd !== undefined && spacingLg
        === undefined) {
      spacing = 0;
      spacingLg = spacingMd;
    }
    ...
    var swiperT = new Swiper($this[0], {
      ...
      pagination: {
        el: [$this.find(".tf-sw-pagination")[0],
      ...
      navigation: {
        nextEl: [
          ...
          $this.closest(".container").find(
              ".group-btn-slider .nav-next-swiper")[0],
        ],
        prevEl: [
          $this.closest(".tf-btn-swiper-main").find(".nav-prev-swiper")[0],
          $this.closest(".container").find(
              ".group-btn-slider .nav-prev-swiper")[0],
        ],
      },
      breakpoints: {
        575: {
    ...
    .on("mouseenter", function () {
      ...
      $(".tf-swiper .card_product--V01.style_2").eq(slideIndex).addClass(
          "active");
    })
    ...
    .on("click", function () {
      var slideIndex = $(this).data("slide");
      $(".tf-swiper .card_product--V01.style_2").eq(slideIndex).toggleClass(
          "clicked");
    });
  });
});
...
$(".modal-quick-view").each(function (index) {
  ...
  function updateModalActiveButton(type, activeIndex) {
    ...
    var currentSlide = $modalRoot.find(".tf-single-slide .swiper-slide").eq(
        activeIndex);
    var currentValue = currentSlide.attr(dataAttr);

    if (currentValue) {
      $modalRoot.find(`${btnClass}[${dataAttr}='${currentValue}']`).addClass(
          "active");
  ...
  function scrollToModalSlide(type, value, color) {
    ...
    var matchingSlides = $modalRoot.find(
        ".tf-single-slide .swiper-slide").filter(function () {
      return $(this).attr(`data-${type}`) === value && $(this).attr(
          "data-color") === color;
    });
    ...
    if (matchingSlides.length > 0) {
      ...
      var fallbackSlides = $modalRoot.find(
          ".tf-single-slide .swiper-slide").filter(function () {
...
if ($(".tf-sw-lookbook").length > 0) {
  ...
  var mobileSm = tfSwLb.data("mobile-sm") !== undefined ? tfSwLb.data(
      "mobile-sm") : mobile;
